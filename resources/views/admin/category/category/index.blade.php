...
@section('content')
    <div class="content-title">
        <div class="pagetitle d-sm-flex align-items-center justify-content-between pb-3">
            <h1>{{ __('app.category') }}</h1>
            <nav>
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin') }}">{{ __('app.admin') }}</a></li>
                    <li class="breadcrumb-item active"><a href="{{ route('admin.category.index') }}">{{ __('app.category') }}</a></li>
                </ol>
            </nav>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card filter-card">
                        ...
                        <div class="d-sm-flex align-items-center justify-content-between">
                            ...
                            <div>
                                <div class="btn-group float-end">
                                    <a href="javascript:void(0)" class="btn btn-primary me-2" onclick="commonShowModal('{{ route('admin.category.create') }}')">{{ __('app.create') }}</a>
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="javascript:void(0)" data-localStorage-name="adminCategories" onclick="showHideShowColumn(this)">{{ __('app.hide_show_column') }}</a>
                                    </div>
                    ...
                    <div class="card-body">
                        <form method="GET" action="{{ route('admin.category.filter') }}" id="filter-form" class="filter-form d-none" onsubmit="commonFilter(); return false">
                                    ...
                                    <div class="form-group mb-3">
                                        <select class="form-select select2" name="module" data-placeholder="{{ __('app.select_module') }}">
                                            @foreach($modules as $module)
                                                <option value="{{ $module->code }}">{{ __('app.'.$module->name) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>

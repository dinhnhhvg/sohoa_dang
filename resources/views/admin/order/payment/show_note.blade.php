            ...
            <div class="tab-content">
                <div class="tab-pane fade p-3 active show" id="tab-list" role="tabpanel">
                    <ul class="timeline-with-icons m-2">
                        @foreach($payment->itemNotes as $note)
                    ...
                    {!! renderSearchEmpty($payment->itemNotes) !!}
                </div>
                ...
                <div class="tab-pane fade p-3" id="tab-create" role="tabpanel">
                    <form method="POST" action="{{ route('admin.payment.update', ['payment' => $payment->id]) }}" onsubmit="commonSubmit(this); return false" enctype="multipart/form-data">
                        ...
                        <div class="row">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-select select2" name="status_id" required data-placeholder="{{ __('app.select_status') }}">
                                        @foreach($statuses as $status)
                                            <option value="{{ $status->id }}" {{ $status->id == $payment->status_id ? 'selected' : '' }}>
                                                {{ __('app.'.$status->name) }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    <label class="form-label">{{ __('app.payment_time') }}</label>
                                    <div class="input-group">
                                        <input type="text" class="form-control datetimepicker" name="payment_time" data-format="d-m-Y H:i"
                                               value="{{ $payment->payment_time?->format('d-m-Y H:i') }}" required placeholder="{{ __('app.payment_time') }}">
                            ...
                            <div class="col-md-12">
                                <div class="form-group mb-3">
                                    <label class="form-label">{{ __('app.invoice') }}<span class="text-danger">*</span></label>
                                    <div class="input-group">
                                        <input type="text" name="image" class="form-control ps-3" placeholder="{{ __('app.select_image') }}" readonly>
                                        <button type="button" class="input-group-text btn btn-primary fa-solid fa-folder-open" title="{{ __('app.select_image') }}"

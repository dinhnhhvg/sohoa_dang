            ...
            <div class="tab-content">
                <div class="tab-pane fade p-3 active show" id="tab-list" role="tabpanel">
                    <ul class="timeline-with-icons m-2">
                        @foreach($judgmentDocument->itemNotes as $note)
                    ...
                    {!! renderSearchEmpty($judgmentDocument->itemNotes) !!}
                </div>

                <div class="tab-pane fade p-3" id="tab-create" role="tabpanel">
                    <form method="POST" action="{{ route('admin.judgment_document.update', ['judgment_document' => $judgmentDocument->id]) }}" onsubmit="commonSubmit(this); return false" enctype="multipart/form-data">
                        @csrf
                        @method('PATCH')
                        <div class="row">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-select select2" name="status_id" required data-placeholder="{{ __('app.select_status') }}">
                                        @foreach($statuses as $status)
                                            <option value="{{ $status->id }}" {{ $status->id == $judgmentDocument->status_id ? 'selected' : '' }}>
                                                {{ __('app.'.$status->name) }}
                                            </option>
                                        @endforeach
                                    </select>

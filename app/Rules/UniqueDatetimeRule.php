...
class UniqueDatetimeRule extends BaseRule
{
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        $startTime = array_shift($parameters);
        $endTime = array_shift($parameters);
        $query->where($startTime, '<', $data[$endTime])
            ->where($endTime, '>', $data[$startTime]);

        if ($this->handleParam($query, $parameters, $data)->first()) {
            $fail(__(
                'validation.custom.unique_datetime',

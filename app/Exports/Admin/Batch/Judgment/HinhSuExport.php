...
class HinhSuExport extends BaseExport implements WithEvents
{
    ...
    public function map(mixed $row): array
    {
        if (isset($row->defendant) && !empty((array)$row->defendant)) {
            $defendantData = [
                ...
                $row->defendant->father_name,
                $row->defendant->mother_name,
                $row->defendant->spouse_name,
                $row->defendant->organization_type,
                $row->defendant->organization_name,
                $row->defendant->organization_business_registration_code,
                $row->defendant->organization_tax_code,
                renderCodeName($row->defendant->organizationProvince),
                renderCodeName($row->defendant->organizationDistrict),
                renderCodeName($row->defendant->organizationWard),
                $row->defendant->organization_address,
                $row->defendant->crime_name,
                $row->defendant->legal_basis,
                renderManyName($row->defendant->mainPenalties, true),
                $row->defendant->main_penalty_value,
                $row->defendant->suspended_sentence,
                $row->defendant->total_court_fee,
                $row->defendant->court_fee_status,
                renderManyName($row->defendant->additionalPenalties, true),
                $row->defendant->additional_penalty_value,
                $row->defendant->prohibited_position,
                $row->defendant->prohibition_duration,
                $row->defendant->prohibition_start_date?->format('d/m/Y'),
                renderManyName($row->defendant->judicialMeasureNames),
                $row->defendant->judicialMeasureIssuer?->name,
                $row->defendant->judicial_measure_start_date?->format('d/m/Y'),
                $row->defendant->judicial_measure_end_date?->format('d/m/Y'),
                $row->defendant->civil_obligation,
                $row->defendant->criminal_record_status,
                $row->defendant->criminal_record_description,
            ];
    ...
    private function headerTree(): array
    {
        return [
            ...
            [
                'label' => __('app.personal_information'),
                'children' => [
                    ['label' => __('app.father_name')],
                    ['label' => __('app.mother_name')],
                    ['label' => __('app.spouse_name')],
                ],
            ],
            [
                'label' => __('app.organization_info'),
                'children' => [
                    ['label' => __('app.organization_type')],
                    ['label' => __('app.organization_name')],
                    ['label' => __('app.business_registration_code')],
                    ['label' => __('app.tax_code')],
                    [
                        'label' => __('app.organization_address'),
                        'children' => [
                            ['label' => __('app.province')],
                            ['label' => __('app.district')],
                            ['label' => __('app.ward')],
            ...
            [
                'label' => __('app.information_on_charges_and_penalties'),
                'children' => [
                    ['label' => __('app.crime_name')],
                    ['label' => __('app.legal_basis')],
                    ['label' => __('app.main_penalty')],
                    ['label' => __('app.main_penalty_value')],
                    ['label' => __('app.suspended_sentence')],
                    ['label' => __('app.court_fee')],
                    ['label' => __('app.court_fee_status')],
                    ['label' => __('app.additional_penalty')],
                    ['label' => __('app.additional_penalty_value')],
                    ['label' => __('app.prohibited_position')],
                    ['label' => __('app.prohibition_duration')],
                    ['label' => __('app.prohibition_start_date')],
                    [
                        'label' => __('app.judicial_measure'),
                        'children' => [
                            ['label' => __('app.judicial_measure_name')],
                            ['label' => __('app.judicial_measure_issuer')],
                            ['label' => __('app.judicial_measure_start_date')],
                            ['label' => __('app.judicial_measure_end_date')],
                        ],
                    ],
                    ['label' => __('app.civil_obligation')],
                    ['label' => __('app.criminal_record_status')],
                    ['label' => __('app.criminal_record_description')],
                ],
            ],
        ];
    }

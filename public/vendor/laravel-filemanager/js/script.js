    ...
    $('#fab').fab({
        buttons: [
            ...
            },
            {
                ...
                label: lang['nav-upload-folder'],
                attrs: {id: 'upload-folder'}
            },
            {
                icon: 'fas fa-folder',
                label: lang['nav-new'],
...
$(document).on('click', '#upload-folder', function () {
...
function toggleActions() {
    ...
    $('[data-action=use]').toggleClass('d-none', !one_selected);
...
function goTo(new_dir) {
    ...
    $("#uploadFolderForm input[name='working_dir']").val(new_dir);
    loadItems();
}
...
function loadItems(page) {
    ...
    performLfmRequest('jsonitems', {show_list: show_list, sort_type: sort_type, page: page || 1}, 'html')
        .done(function (data) {
            ...
            if (response.view_tree) {
                $('#tree').html(response.view_tree);
            }
            ...
            $("#uploadFolderForm input[name='working_dir']").val(working_dir);
...
function preview(items) {
    ...
    items.forEach(function (item, index) {
        ...
        window.console.log(item.icon);
        if (item.icon === 'pdf' || item.icon === 'PDF') {
            carouselItem.find('.carousel-image').css({
                width: '100%',
                height: 'calc(95vh - 50px)',
                background: 'none'
            }).html(`<iframe src="${item.url}#toolbar=0" style="width:100%;height:100%;border:none"></iframe>`);
            if (window.parent && typeof window.parent.editByFilePath === 'function') {
                window.parent.editByFilePath(item.url);
            }
            $('#tree').remove();
            ...
            carouselItem.find('.carousel-image').css({
                ...
                height: 'auto',
                background: 'none'
            }).html('<img src="' + item.url + '?timestamp=' + item.time + '" style="width: 100%; height: 100%" alt="image">');
        } else {
            carouselItem.find('.carousel-image').css('width', 'auto').append($('<div>').addClass('mime-icon ico-' + item.icon));
        }
        carouselItem.find('.carousel-label').attr('target', '_blank').attr('href', item.url).text(item.name).append($('<i class="fas fa-external-link-alt ml-2"></i>'));
        carousel.children('.carousel-inner').append(carouselItem);
        var carouselIndicator = indicatorTemplate.clone().addClass(index === 0 ? 'active' : '').attr('data-slide-to', index);
...
function showFile(carousel) {
    $('#content').parent().addClass('col-3').removeClass('col-12');
    $('#file-show').parent().addClass('col-9').removeClass('d-none');
    $('#file-show').html(carousel);
    $('[data-action=use]').addClass('d-none');
    $('[data-action=preview]').addClass('d-none');
}

function hideFile() {
    $('#content').parent().addClass('col-12').removeClass('col-3');
    $('#file-show').parent().addClass('d-none').removeClass('col-9');
    $('#file-show').html('');
}

let alohubData = window.localStorage.getItem('alohubData');
alohubData = alohubData.replace(/&quot;/g, '"');
alohubData = JSON.parse(alohubData);
...
var callOptions = {
    ...
    extraHeaders: [
        `X-CRM-INFO: ${alohubData.xCrmInfo}`
    ],
};
...
window.onload = function () {
    ...
    if (alohubData.txtDisplayName !== null && alohubData.txtDisplayName !== '') {
        usingAutoLogin = 1;
    }
    ...
    if (usingAutoLogin === 1) {
        ...
        $('#alohub_sipml5').addClass('d-none');
        $('#alohub_sipml5_dashboard_notification').removeClass('d-none');
    } else {
        loadContentPhone();
    }
}

function showAlohubCallModal(phone, e = '#alohub_sipml5') {
    if (usingAutoLogin !== 1) {
        showNotification('error', 'Bạn chưa được kết nối với tổng đài!');
        return;
    }

    $(e).find('#txtPhoneNumber').val(phone);
...
function addHtmlLogin() {
    if (usingAutoLogin === 1) {
        ...
        document.getElementById('alohub_sipml5').innerHTML += "<div id=\"alohub_login_content\">\n" +
            "        <button id=\"btnOpenLoginForm\" class=\"alohub_button-call d-none\" onclick=\"onTogglePhone();\">\n" +
...
function addHtml() {
    document.getElementById('alohub_sipml5').innerHTML += ' <!--Form Login\'-->\n' +
        ...
        '        <div class="d-flex">\n' +
        '            <input type="password" id="txtPhoneNumber" readonly>\n' +
        ...
        '                    </svg>\n' +
        '                </span>\n' +
        '        <span class="alohub_icon_minimize d-none" onclick="minimizeContent();">\n' +
        '                    <svg width="25" height="25" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">\n' +
        '                        <g id="icomoon-ignore">\n' +
        '                        </g>\n' +
        '                        <path d="M6.576 6.576c-5.205 5.205-5.205 13.643 0 18.849s13.643 5.205 18.849-0c5.206-5.206 5.206-13.643 0-18.849s-13.643-5.205-18.849 0zM24.67 24.67c-4.781 4.781-12.56 4.781-17.341 0s-4.781-12.56 0-17.341c4.781-4.781 12.56-4.781 17.341 0s4.78 12.56-0 17.341z"\n' +
        ...
        '        </div>\n' +
        '    </div>\n' +
        '    <!--Form Ringing(có 2 giao diện khi người dùng thay đổi)-->\n' +
        '    <div id="alohub_ringing_content">\n' +
        '            <span class="alohub_icon_full" onclick="minimizeContent();">\n' +
        '                    <svg width="25" height="25" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" fill="none"\n' +
        '                         stroke="customColor">\n' +
        '                        <polyline points="36 48 48 48 48 36"/>\n' +
        ...
        '                    Số điện thoại\n' +
        '                </div>\n' +
        '                <div id="txtPhoneRinging"></div>\n' +
        '                <div class="alohub_title_calling">\n' +
        '                    Đang gọi đến ...\n' +
        '                </div>\n' +
        '            </div>\n' +
        '            <div class="alohub_action_call">\n' +
        '                <button class="alohub_button-answer" onclick="onAnswer();">\n' +
        '                    <svg width="25" height="25" viewBox="0 0 24 24" fill=\\"none\\" xmlns="http://www.w3.org/2000/svg">\n' +
        '                        <path d="M9 16C2.814 9.813 3.11 5.134 5.94 3.012l.627-.467a1.483 1.483 0 0 1 2.1.353l1.579 2.272a1.5 1.5 0 0 1-.25 1.99L8.476 8.474c-.38.329-.566.828-.395 1.301.316.88 1.083 2.433 2.897 4.246 1.814 1.814 3.366 2.581 4.246 2.898.474.17.973-.015 1.302-.396l1.314-1.518a1.5 1.5 0 0 1 1.99-.25l2.276 1.58a1.48 1.48 0 0 1 .354 2.096l-.47.633C19.869 21.892 15.188 22.187 9 16z"\n' +
        '                              fill="customColor"/>\n' +
        '                    </svg>\n' +
        '                    <span class="alohub_button-title">Nghe máy</span>\n' +
        '                </button>\n' +
        '                <button class="alohub_button-hangup" onclick="onHangUp();">\n' +
        '                    <svg width="25" height="25" viewBox="0 0 24 24" fill=\\"none\\" xmlns="http://www.w3.org/2000/svg">\n' +
        '                        <path d="M9 16C2.814 9.813 3.11 5.134 5.94 3.012l.627-.467a1.483 1.483 0 0 1 2.1.353l1.579 2.272a1.5 1.5 0 0 1-.25 1.99L8.476 8.474c-.38.329-.566.828-.395 1.301.316.88 1.083 2.433 2.897 4.246 1.814 1.814 3.366 2.581 4.246 2.898.474.17.973-.015 1.302-.396l1.314-1.518a1.5 1.5 0 0 1 1.99-.25l2.276 1.58a1.48 1.48 0 0 1 .354 2.096l-.47.633C19.869 21.892 15.188 22.187 9 16z"\n' +
...
function minimizeContent() {
    ...
    alohub_ringing_content.classList.toggle("minimize");
}
...
function loadContentPhone() {
    ...
    alohub_ringing_content.style.display = 'none';
...
function loadContentPhoneCalling() {
    alohub_call_dial.style.display = 'none';
    alohub_calling_content.style.display = 'none';
    alohub_answer_content.style.display = 'none';
    alohub_ringing_content.style.display = 'block';
    hiddenLogin();
}
...
function loadCredentials() {
    if (window.localStorage) {
        ...
        if ((s_value = alohubData.txtWebsocketServerUrl)) {
            txtWebsocketServerUrl.value = s_value;
        }
        if ((s_value = alohubData.txtPublicIdentity)) {
            txtPublicIdentity.value = s_value;
        }
        if ((s_value = alohubData.txtPassword)) {
            txtPassword.value = s_value;
        }
        if ((s_value = alohubData.txtDisplayName)) {
...
function addPhoneNumber(number) {
    txtPhoneAnswer.innerHTML = '**********';
    txtPhoneCalling.innerHTML = '**********';
    txtPhoneRinging.innerHTML = '**********';
}
...
function onRegister() {
    ...
    coolPhone.on('newRTCSession', function (e) {
        ...
        $('#alohub_sipml5').removeClass('d-none');
        $('#txtPhoneNumber').val(session.remote_identity.uri.user);

        session.on('ended', function (e) {

            update_history_call(session.remote_identity.uri.user);
        ...
        session.on('connecting', function (e) {
            ...
            console.log(e.request.getHeader('X-CRM-INFO'));
    ...
    coolPhone.on('message', function (e) {
        console.log('Received SIP message:', e.data);
    });

    coolPhone.on('transportCreated', function (e) {
        console.log('Transport created:', e.transport);
        // Now you have access to the transport object
    });
...
function update_history_call(phone) {
    console.log('Update history');
    return;

    $.ajax({
        url: `${window.location.origin}/crm/call/add_user_id`,
        type: 'POST',
        dataType: 'JSON',
        data: {
            phone: phone
